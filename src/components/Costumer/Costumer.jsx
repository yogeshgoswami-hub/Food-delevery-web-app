          ...
          <div className="flex md:flex-row flex-col gap-10 justify-center items-center m-15 ">
            {cost.map((item, index) => (
              <div
                ...
                className="p-5 rounded-xl transition-all duration-200 cursor-pointer hover:shadow-xl active:shadow-xl ">
                <img
                  src={item.image}
                  className="md:w-35 md:h-35 h-40 w-40 mx-auto rounded-full mb-15"
                />
                <h3 className="text-2xl text-center font-semibold text-zinc-800">
                  {item.name}
                </h3>
                <p className="text-zinc-800 text-lg text-center mt-2">{item.para}</p>
